...
export function ArticleForm(props: ArticleFormProps) {
  ...
  const { user, categories, articles, users, setUser } = useStore((s) => s);
  const { refresh } = useData();
  const [isReady, setIsReady] = useState(
    article.readyToPublish ? article.readyToPublish : false,
  );
  ...
  return (
    <form onSubmit={handleFormSubmit} className="new-article-form">
      <div>
        ...
        {isReady && (
          <div className="lineContainer">
            <div
              className="bButton checkBoxSelected"
      ...
      <button type="submit">
        {editing ? "Submit Changes" : "Post Article"}
      </button>
    </form>
  );
}
