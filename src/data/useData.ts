...
import type { Meme } from "../state/useStore";
...
export function useData() {
  const { categories, articles, articlesLoaded, usersLoaded,
    ...
    resourcesLoaded, setResources, setResourceTypes, setResourcesLoaded,
    memesLoaded, setMemes, setMemesLoaded } = useStore((s) => s);
  ...
  const fetchMemes = useCallback(async () => {
    ...
    try {
      ...
      const data: Meme[] = res.data.data;
      setMemes(data);
    } catch (err) {
      console.error("Failed to fetch memes:", err);
    }
  }, [memesLoaded, setMemesLoaded]);

  const killMeme = useCallback(async (killID: string) => {
    try {
      await axios.delete(`/api/memes/${killID}`);
    } catch (err) {
      console.error("Failed to delete meme:", killID);
    }
  }, []);
